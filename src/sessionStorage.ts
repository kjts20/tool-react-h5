...
// 获取sessionStorage
const getSessionStoragee = function (key) {
...
/**
 * sessionStorage实现CommonStorage
 */
export const sessionStorageImplCommonStorage: IStorageApi = {
    ...
    getStorage(options: IGetStorageOptions) {
        ...
        try {
            const data = getSessionStoragee(key);
            if (success) {
                success({ data });
            }
            return true;
...
// 导出默认缓存
export const storage = new CommonStorage(sessionStorageImplCommonStorage);
